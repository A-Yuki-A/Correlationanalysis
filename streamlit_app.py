...
st.markdown("""
**外れ値の定義（IQR法）**  
四分位範囲（IQR = Q3 − Q1）の 1.5倍より外にあるデータを外れ値とします。  
ここで Q1 は下位25%、Q3 は上位25%の値です。

---

**スピアマン順位相関とは**  
データの「大小関係」（順位）を使って相関の強さを測る方法です。  
値そのものではなく、順位に変換して計算するため、外れ値や極端な値の影響を受けにくい特徴があります。  

**例**  
国語と数学のテストで、点数はバラバラでも、生徒の順位がほぼ同じであればスピアマン順位相関は高くなります。  
例えば  
- 国語の点数順位: 1位, 2位, 3位, 4位, 5位  
- 数学の点数順位: 1位, 2位, 3位, 4位, 5位  
この場合、スピアマン順位相関は「1.0（完全に一致）」になります。
""")
